...
func RegisterAdvertiserRoutes(router chi.Router, db *sql.DB) {
	...
	router.Route("/advertisers", func(r chi.Router) {
		r.With(authmw.RequirePermission(db, "advertisers.read")).Get("/search", advertiserHandler.SearchAdvertisers)
		r.With(authmw.RequirePermission(db, "advertisers.read")).Get("/", advertiserHandler.ListAdvertisers)
		r.With(authmw.RequirePermission(db, "advertisers.write")).Post("/", advertiserHandler.CreateAdvertiser)
		r.Route("/{id}", func(r chi.Router) {
			r.With(authmw.RequirePermission(db, "advertisers.read")).Get("/", advertiserHandler.GetAdvertiser)
